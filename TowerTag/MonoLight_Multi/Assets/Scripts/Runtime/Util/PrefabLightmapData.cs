...
public class PrefabLightmapData : MonoBehaviour {
    ...
    [SerializeField] private Texture2D[] m_Lightmaps;
    ...
    void Awake() {
        ...
        for (int i = 0; i < m_Lightmaps.Length; i++) {
            ...
            if (!exists) {
                ...
                var newLightmapData = new LightmapData {lightmapColor = m_Lightmaps[i]};
...
#if UNITY_EDITOR
    [MenuItem("Lightbake/Bake Prefab Lightmaps")]
    static void GenerateLightmapInfo() {
        ...
        foreach (var instance in prefabs) {
            ...
            if (targetPrefab != null) {
                //UnityEditor.Prefab
                PrefabUtility.SaveAsPrefabAsset(gameObject, AssetDatabase.GetAssetPath(targetPrefab));
            }
        }
    }
    ...
    static void GenerateLightmapInfo(GameObject root, List<RendererInfo> rendererInfos, List<Texture2D> lightmaps) {
        ...
        foreach (MeshRenderer renderer in renderers) {
            if (renderer.lightmapIndex != -1) {
                RendererInfo info = new RendererInfo {renderer = renderer, lightmapOffsetScale = renderer.lightmapScaleOffset};
