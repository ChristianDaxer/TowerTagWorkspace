...
namespace UnityEngine.UI.Extensions {
    ...
    public class AutoFlowLayout : LayoutGroup {
        ...
        private void CalculateCellSize() {
            ...
            float iColumn = rectTransform.rect.width >= rectTransform.rect.height ? Mathf.CeilToInt(Mathf.Sqrt(childCount)) : Mathf.FloorToInt(Mathf.Sqrt(childCount));
        ...
        public float SetLayout(int axis, bool layoutInput) {
            ...
            float groupSize = 0;
            float workingSize = 0;
            var tmpPadding = padding;
            if (startAxis == Axis.Horizontal) {
                groupSize = groupHeight;
                workingSize = groupWidth - tmpPadding.left - tmpPadding.right;
                if (IsLowerAlign) {
                    offset = tmpPadding.bottom;
                    counterOffset = tmpPadding.top;
                }
                else {
                    offset = tmpPadding.top;
                    counterOffset = tmpPadding.bottom;
                }
                ...
                workingSize = groupHeight - padding.top - tmpPadding.bottom;
                if (IsRightAlign) {
                    offset = tmpPadding.right;
                    counterOffset = tmpPadding.left;
                }
                else {
                    offset = tmpPadding.left;
                    counterOffset = tmpPadding.right;
                }
                ...
                if (currentBarSize + childSize > workingSize && !(Mathf.Approximately(currentBarSize + childSize, workingSize))) {
                    ...
                    if (!layoutInput) {
                        if (startAxis == Axis.Horizontal) {
                            ...
                            LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize, tmpPadding.left, newOffset, axis);
                        }
                        else if (startAxis == Axis.Vertical) {
                            float newOffset = CalculateColHorizontalOffset(groupSize, offset, currentBarSpace);
                            LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize, newOffset, tmpPadding.top, axis);
                        }
                    }
            ...
            if (!layoutInput) {
                if (startAxis == Axis.Horizontal) {
                    ...
                    currentBarSize -= spacingBetweenElements;
                    LayoutRow(_itemList, currentBarSize, currentBarSpace, workingSize - (ChildForceExpandWidth ? 0 : spacingBetweenElements), tmpPadding.left, newOffset, axis);
                }
                else if (startAxis == Axis.Vertical) {
                    float newOffset = CalculateColHorizontalOffset(groupWidth, offset, currentBarSpace);
                    currentBarSize -= spacingBetweenElements;
                    LayoutCol(_itemList, currentBarSpace, currentBarSize, workingSize - (ChildForceExpandHeight ? 0 : spacingBetweenElements), newOffset, tmpPadding.top, axis);
                }
            }
