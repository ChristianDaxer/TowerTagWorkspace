...
namespace Valve.VR
{
    public class SteamVR_LoadLevel : MonoBehaviour
    {
        ...
        System.IntPtr loadingScreenPtr;
        ...
        [SerializeField] Texture[] gameTips;

        //Used to cache the native textures pointer
        System.IntPtr[] nativeTextureTips;
        ...
        static SteamVR_LoadLevel _instance = null;

        public static SteamVR_LoadLevel Loader => _instance;
        ...
        private void Awake() {
            ...
            if (gameTips.Length > 0) {
                nativeTextureTips = new System.IntPtr[gameTips.Length];

                for (int i = 0; i < gameTips.Length; i++) {
                    nativeTextureTips[i] = gameTips[i].GetNativeTexturePtr();
                }
            }

            loadingScreenPtr = System.IntPtr.Zero;
        }
            ...
            float r = 0.0f, float g = 0.0f, float b = 0.0f, float a = 1.0f)
        {
            ...
            SteamVR_LoadLevel loader = (_instance == null) ? new GameObject("loader").AddComponent<SteamVR_LoadLevel>() : _instance;

            loader.loadingScreenPtr = (loader.nativeTextureTips.Length > 0) ? loader.nativeTextureTips[rnd.Next(loader.nativeTextureTips.Length)] : System.IntPtr.Zero;
        ...
        IEnumerator LoadLevel()
        {
            ...
            while((overlay = OpenVR.Overlay) == null) {
                yield return new WaitForEndOfFrame();
            }

                // Optionally create our loading screen overlay.
            if ((loadingScreen != null || loadingScreenPtr != System.IntPtr.Zero) && overlay != null)
            {
                ...
                {
                    var texture = new Texture_t();
                    texture.handle = (loadingScreenPtr != System.IntPtr.Zero) ? loadingScreenPtr : loadingScreen.GetNativeTexturePtr();
                    texture.eType = SteamVR.instance.textureType;
                    texture.eColorSpace = EColorSpace.Auto;
            ...
            if (overlay != null) {
                if (progressBarOverlayHandle != OpenVR.k_ulOverlayHandleInvalid)
                    overlay.ShowOverlay(progressBarOverlayHandle);
                if (loadingScreenOverlayHandle != OpenVR.k_ulOverlayHandleInvalid)
                    overlay.ShowOverlay(loadingScreenOverlayHandle);
            }
