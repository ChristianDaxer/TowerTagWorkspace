...
public class ScreenshotCompanion : MonoBehaviour
{
    ...
    public delegate void PrepareScreenShot(ScreenshotCompanion sender, Camera camera);
    public event PrepareScreenShot ScreenShotCompanionPrepareScreenShot;

    public delegate void TakeUpScreenShot(ScreenshotCompanion sender, Camera camera);
    public event TakeUpScreenShot ScreenShotCompanionTakeUpScreenShot;
    ...
    {
        var camAdded = false;
        foreach (var cameraObject in list)
        {
            if (cameraObject.cam != null) continue;
            cameraObject.cam = camera;
            camAdded = true;
        }
        if (camAdded) return;
        var newCam = new CameraObject {cam = camera};
        list.Add(newCam);
    }
    ...
    {
        ScreenShotCompanionPrepareScreenShot?.Invoke(this, list[id].cam.GetComponent<Camera>());
        for (int i = 0; i < list.Count; i++)
        {
            if (list[i].cam != null)
                list[i].cam.SetActive(false);
        ...
        Debug.Log("Screenshot saved to: " + fileName);
        ScreenShotCompanionTakeUpScreenShot?.Invoke(this, list[id].cam.GetComponent<Camera>());
        postCapture();
    }
