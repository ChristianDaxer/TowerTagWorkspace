...
namespace Photon.Voice.Unity
{
    ...
    public class Speaker : VoiceComponent
    {
        ...
        [SerializeField] private bool playbackOnlyWhenEnabled;
...
#else
            Func<IAudioOut<float>> factory = () => new UnityAudioOut(this.GetComponent<AudioSource>(), this.Logger,
                string.Empty, this.Logger.IsInfoEnabled);
#endif
        ...
        internal bool OnRemoteVoiceInfo(RemoteVoiceLink stream)
        {
            ...
            ) // early avoid possible crash due to ArgumentException in AudioClip.Create inside UnityAudioOut.Start
            {
                if (this.Logger.IsErrorEnabled)
                {
                    this.Logger.LogError(
                        "Received voice info channels is not expected: {0} <= 0, cancelled linking to {1}/{2}",
        ...
        internal void OnRemoteVoiceRemove()
        {
            if (this.Logger.IsDebugEnabled)
            {
                this.Logger.LogDebug("OnRemoteVoiceRemove {0}/{1}", this.remoteVoiceLink.PlayerId,
                    this.remoteVoiceLink.PlayerId);
            }
        ...
        private bool StartPlaying()
        {
            ...
            if (this.Logger.IsDebugEnabled)
            {
                this.Logger.LogDebug("StartPlaying {0}/{1}", this.remoteVoiceLink.PlayerId,
                    this.remoteVoiceLink.PlayerId);
            }
