...
Shader "_OwnShader/ProjectorAlphaBlended_SimpleAttenuation" 
{
	Properties 
	{
	...
	Category 
	{
		...
		SubShader 
		{
			Pass 
			{
				...
				{
					fixed4 tex = tex2D(_MainTex, saturate(i.texcoord.xy));
					fixed4 res = lerp(0, tex * _TintColor * _ColorStrength, 1 - abs(i.uvFalloff.z));
