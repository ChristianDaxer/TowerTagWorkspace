...
Shader "Hidden/BlurAndFlares" {
	...
	v2f_blur vertWithMultiCoords2 (appdata_img v) {
		...
		o.uv.xy = uv;
		o.uv01 = uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1);
		o.uv23 = uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 2.0;
		o.uv45 = uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 3.0;
		o.uv67 = uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 4.0;
		o.uv67 = uv.xyxy + _Offsets.xyxy * half4(1,1, -1,-1) * 5.0;
		return o;  
	}
	...
	v2f_opts vertStretch (appdata_img v) {
		...
		float2 uv = UnityStereoScreenSpaceUVAdjust(v.texcoord.xy, _MainTex_ST);
		o.uv[0] = uv;
		o.uv[1] = uv + b * 2.0 * _Offsets.xy;
		o.uv[2] = uv - b * 2.0 * _Offsets.xy;
		o.uv[3] = uv + b * 4.0 * _Offsets.xy;
		o.uv[4] = uv - b * 4.0 * _Offsets.xy;
		o.uv[5] = uv + b * 6.0 * _Offsets.xy;
		o.uv[6] = uv - b * 6.0 * _Offsets.xy;
		return o;
	}
	...
	v2f_opts vertWithMultiCoords (appdata_img v) {
		...
		float2 uv = UnityStereoScreenSpaceUVAdjust(v.texcoord.xy, _MainTex_ST);
		o.uv[0] = uv;
		o.uv[1] = uv + 0.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[2] = uv - 0.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[3] = uv + 1.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[4] = uv - 1.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[5] = uv + 2.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		o.uv[6] = uv - 2.5 * _MainTex_TexelSize.xy * _Offsets.xy;
		return o;
	}	
