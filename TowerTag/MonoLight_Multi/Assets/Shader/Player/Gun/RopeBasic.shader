		...
		/*
			...
			*/
			v2hull vert(appdata v)
			{
				...
				float4 spline = select_spline_IndividualVars_time(t);
				*/
				float slurp_time = 0;// dot(spline, t_arg);
				...
				o.P = UnityWorldToClipPos(v.vertex);
				/*o.VPOS = UnityWorldToClipPos(v.vertex.xyz);
				*/
				o.UV = v.UV;
				o.slurp_time = slurp_time;
...
//				
			...
			*/
			...
			fixed4 frag (v2hull i) : SV_Target
			{
				return fixed4(_TintColor.rgb, 1);
				// original Till
			...
			//
				...
				return fixed4(_TintColor.rgb, alpha);// spline_res );
				
			}
			*/
			fixed4 frag(v2hull i) : COLOR
			{
				return fixed4(_TintColor.rgb, 1);
			}
			ENDCG
